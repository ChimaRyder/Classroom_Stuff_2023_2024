...
public class Main {
    ...
    public static void giveRaise(List<Person> persons, String manager, String employee, double salary)  {
        ...
        for (Person p : persons)
        {
            if (p.getName().equals(manager))
            {
                if (p instanceof Manager) {
                    ...
                    {
                        if (q.getName().equals(employee))
                        {
                            if (q instanceof Employee) {
                                ((Manager) p).giveRaise(((Employee) q), salary);
                                exists = true;
                                break;
                            } else {
                                throw new ClassCastException(q.getName() + " is not an employee");
                            }
                        }
                    }
                } else {
        ...
        if (!exists)
        {
            throw new NoSuchElementException(finding + " does not exist");
        }
    }
        ...
        for (Person p : persons)
        {
            ...
            if (p.getName().equals(developer))
            {
                if (p instanceof Developer) {
                    ...
                    {
                        finding = manager;
                        if (q.getName().equals(manager))
                        {
                            if (q instanceof Manager) {
                                ((Developer) p).setProjectManager(((Manager) q));
                                exists = true;
                                break;
                            } else {
                                throw new ClassCastException(q.getName() + " is not a manager");
                            }
                        }
                    }
                    break;
    ...
    public static String customerSpeak(List<Person> persons, String customer, String employee) {
        boolean exists = false;
        String finding = null, speakout = null;
            ...
            if (p.getName().equals(customer))
            {
                ...
                if (p instanceof Customer) {
                    for (Person q : persons)
                    {
                        ...
                        {
                            if (q instanceof Employee) {
                                speakout = ((Customer) p).speak(((Employee) q));
                                exists = true;
                                break;
                            } else {
                                throw new ClassCastException(q.getName() + " is not an employee");
